...
## 9. iOS UI 高保真克隆实现流程（2026-09-09 起，用户裁决——每次 UI 工作必须遵循）

> 背景：首轮 iOS UI 按"对原型的记忆"实现，与用户 7 天打磨确认的高保真原型差距明显，违反 R-G5。
> 根因 = 凭记忆实现而非从冻结原型提取规格。本节把纠正方法固化为硬性流程。

**核心原则：冻结的原型 `docs/ios/design/prototype/index.html` 是 UI 的唯一事实源；实现 = 提取 + 克隆 + 截图对照，禁止凭记忆/近似。**

每页 UI 的强制流程（缺一步不得宣布完成）：

1. **提取规格卡**：从原型源码提取该页规格——CSS Token（色板/圆角 10/8/18/阴影）、组件结构顺序（DOM 即结构）、精确数值（padding 13/14、字号 11–23px 阶梯、行高、字重）、逐字文案（i18n 字典）。提取结果落 `docs/ios/design/ui-spec/`（每页一份）。
2. **组件库克隆**：DesignSystem 组件（Card/FieldTile/SectionHeader/SessionControlCard/OpCard/StatePill/Banner/Steps/Dialog/Sheet/SaveBar…）参数**逐条来自原型 CSS**，禁止用 SwiftUI 默认样式近似。页面 = 组件组装，不允许页内私调样式。
3. **截图对照验收**：完成页必须生成并排对照图（模拟器/Mac 390×844 截图 vs 原型同尺寸截图，存 `docs/ios/design/comparisons/<page>.png`），自查差异清零后方可提交；用户抽查通过该页才算关闭。
4. **提交纪律**：UI 提交信息注明对照截图路径；**原型变更 → 同一提交内更新规格卡与实现**（与 §8 第 3 条同源纪律一致）。
5. **未实现页面**（OTA/配置/任务/校准/Debug/设置/定位 UI 细节）一律按本流程开发；已实现页面在 M4 收尾前按本流程逐页重审重写。

工具约定：原型截图用浏览器打开 `index.html`（390×844 手机框内），模拟器截图用 `xcrun simctl io <device> screenshot`；对照图左右并排标注页码与日期。
