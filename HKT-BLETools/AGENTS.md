...
| Android SDK | `D:\Android\sdk` | 即 `local.properties` 的 `sdk.dir`（该文件**不在 VCS**，丢失需重建） |
...
**首选 `build.ps1`**（自动设 `JAVA_HOME=D:\Java\jdk-17` 后调 `gradlew.bat`，规避 PATH 上无效的 jdk-21）：
...
**产物路径**：`app/build/outputs/apk/<flavor>/<buildType>/`
...
**已解决**：
- `settings.gradle.kts` 已加**阿里云 Maven 镜像**（`google`/`public`/`gradle-plugin`，官方源作 fallback）
- `gradle/wrapper/gradle-wrapper.properties` 的 `distributionUrl` 已改为**腾讯云 gradle 镜像**
...
**一键启动**：双击 [启动模拟器.bat](启动模拟器.bat)（自动启动→等开机→装最新 prod-debug APK→启动 app）。
...
"$BT/apksigner.bat" sign --ks key.jks --ks-key-alias hkt --out signed.apk aligned.apk
```
