...
- 结构化日志：OSLog（开发）+ 进程内环形文件缓冲（诊断导出），覆盖扫描启停/目标发现/连接变化/服务发现/订阅结果/写入/响应/OTA 阶段/错误恢复九类事件；每条记录含时间戳与等级；导出文件头附环境信息（App 版本号与构建号、机型、iOS 版本、当前语言），保证现场反馈的日志可定位到具体版本；记录字段为值类型快照；敏感数据（无密钥类数据，仅设备标识）白名单制，导出前人工可见内容固定模板。
