...
| 编号 | 补充需求 | 依据 | 落点 |
| --- | --- | --- | --- |
| S-1 | 1 s 状态轮询定位为**保活硬约束**：固件在连接后 5 s 无交互即进睡眠、60 s 未连接强制睡眠；连接成功必须自动发起首轮 `0xFF` 查询，任何轮询暂停策略间隔 ≤4 s，否则显式断开 | `control_center.h` `WAIT_SLEEP_TIME_DELAY=5`/`MAX_WAIT_SLEEP_TIMEOUT=60` | 架构大纲 §7.1；TC-ST-001~004 |
