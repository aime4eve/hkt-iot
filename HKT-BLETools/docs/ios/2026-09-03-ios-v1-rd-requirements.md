...
3. 读取 RSSI。
4. 支持信号强度阈值过滤。
5. 支持按设备名称前缀过滤（MPS/SVC/UDS/EPS 四个前缀可独立勾选，设置持久化；未命名设备天然被排除——2026-09-08 用户裁决，替代原"过滤无名称设备"）。
6. 扫描结果去重。
...
iOS v1 即包含目标设备定位主 UI（2026-09-07 用户裁决选 B，翻案原 9-6"v1 不含"结论）：扫描页提供⌖入口，扫描设备标签二维码或手工输入 16 位 DevEUI，取后 6 位匹配广播名，命中即自动停扫并直连；3 轮未命中给出"未找到"与重试/退出。扫码需相机权限（见 §9.4）。交互设计以高保真原型 P-01b 为准（R-2/SP-2，TC-ST-014~017）。
...
4. App 进入后台导致扫描受限。
5. 长时间 OTA 期间的屏幕锁定风险。
6. 相机权限（仅目标设备定位扫码使用，2026-09-07 随定位功能进 v1 新增）：未请求时弹系统弹窗；被拒时提供系统设置引导；手输 DevEUI 路径不受相机权限影响。
...
| --- | --- | --- |
| BLE 扫描 | 已实现 | v1 必须 |
| RSSI / 名称前缀过滤 | Android 仅空名称过滤；名称前缀过滤为 iOS 增强（用户 2026-09-08） | v1 必须 |
| 手工 DevEUI | 已实现 | v1 必须（2026-09-07 裁决，R-2/SP-2） |
| 二维码 / 条码 | 已实现 | v1 必须（2026-09-07 裁决，R-2/SP-2；需相机权限） |
| 连接与会话守护 | 已实现 | v1 必须 |
| 状态查询 | 已实现 | v1 必须 |
...
以下问题已于 M0（2026-09-05）全部裁决为工作决策，结论如下（含 2026-09-07 的一次翻案）：

1. **参数配置 UI 是否进入 v1**：进入 v1，排期 M8（Q1 修订；清单 R-16/SP-16）。
2. **状态展示字段范围**：按设备家族能力全集展示（UDS100 13 项 / DC200Family 4 项 / SVC100 9 项），另设 DC200Family 技术参数页（R-5/R-27）。
3. **Debug 原始命令入口是否进入 v1**：进入 v1，隐藏入口=关于页连点版本 7 次，dev 构建默认开启（Q3；R-19/SP-19）。
4. **手工 DevEUI 与二维码入口是否进入 v1**：~~v1 不开放（Q4 2026-09-06）~~ → **进入 v1（2026-09-07 用户裁决选 B，翻案 Q4）**：扫码/手输 DevEUI→后 6 位匹配广播名→命中自动连接（R-2/SP-2，TC-ST-014~017）；需新增相机权限（§9.4-6）。
5. **最低支持 iOS 版本**：iOS 17.0（@Observable 直接满足状态同步硬性要求）。
6. **最低支持 iPhone 机型**：iPhone XS/XR 起、仅 iPhone；测试矩阵含一台小屏机型。
7. **每类设备的标准测试固件**：5 条基线（MPS100/EPS100/UDS100/SVC100+旧版本对）；EPS100 依赖固件组提供 `#if 1` 构建产物（外部项，M6 真机前到位）。
8. **OTA 后预期版本格式**：重连后经 0xFF 查询读 TLV 0x01 硬件+软件版本字节与期望版本比对；期望版本 v1 采用"确认弹窗预填"制。
9. **分发方式**：研发期开发签名直装，对外 TestFlight；M4 开工前必须到位 1 台签名真机。
10. **App 名称与 Bundle ID**：显示名 `HKT BLETools`，Bundle ID `com.hkt.ble.bletools.ios`。
