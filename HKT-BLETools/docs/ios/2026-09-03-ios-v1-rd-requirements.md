...
| --- | --- |
| 文档版本 | v0.6 Draft |
| 日期 | 2026-09-03（v0.6 增补 2026-09-04，见第 18 章） |
| 产品 | HKT BLETools iOS |
...
7. 每个错误状态附带建议动作（2026-09-04 评审回写，原为 UI 计划单方口径）。
8. OTA 结束生成可导出的升级报告（升级前后版本、各阶段耗时、失败原因），作为现场支持与验收留痕材料（同上回写）。
...
| 编号 | 验收项 | 通过条件 |
| --- | --- | --- |
| A-00 | UI 设计基线 | 信息架构、关键流程、高保真稿、组件状态、中英文文案和 OTA 原型通过评审；错误附建议动作与 OTA 升级报告两条验收标准（§3.3 第 7/8 条）一并核对 |
...
## 18. 补充需求（v0.6，2026-09-04 稳定性推演产出）

以下条目来自对地磁设备（DC200Family）BLE 稳定性的固件级思想实验，均已在架构大纲与测试用例基线中落实设计；此章为需求侧登记，编号 S-xx：

| 编号 | 补充需求 | 依据 | 落点 |
| --- | --- | --- | --- |
| S-1 | 睡眠语义按连接态区分（2026-09-04 评审更正）：未连接广播态 5 s 无交互进睡眠（`WAIT_SLEEP_TIME_DELAY=5`）；**连接态固件低功耗路径提前返回、默认不睡眠**，受 `ble_sleep_mode` 配置影响（实际取值 M3 核实）。连接成功仍必须自动发起首轮 `0xFF` 查询；1 s 轮询保留为状态刷新与保活手段；轮询暂停策略须评估设备侧行为并显式恢复 | `control_center.h`；`control_center.c` 连接态跳过睡眠路径 | 架构大纲 §7.1；TC-ST-001~004 |
| S-2 | LoRa 上行与 BLE 同主循环，会话中可能出现秒级响应延迟；错误处理必须区分"响应慢"与"无响应"，延迟事件记入诊断日志 | `control_center.c` `dataReportTimestamp` 上报触发 | 架构大纲 §5.3/§7.1；TC-ST-005 |
