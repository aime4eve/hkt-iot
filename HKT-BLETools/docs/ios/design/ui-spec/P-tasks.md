...
body
  ├─ rtCard 实时任务卡（.card）：标题「实时任务 · 0x03」(700) —— 状态机四态（2026-09-14 交互裁决）
  │    ├─ fieldgrid 2 列：阀门（双阀/阀1/阀2 chips）+ 动作（开阀/关阀 chips）
  │    ├─ 水平 2 列 .field：持续时间(s) 输入+durHint / 脉冲数 输入
  │    ├─ [空闲] .btn「立即执行」
  │    ├─ [发送中] info banner「⏳ 发送中…」+ 表单锁定（半透明禁点）+ 按钮禁用
  │    ├─ [设备执行中] info banner「● 设备执行中 — 阀x · 开/关 · 脉冲 N」
  │    │      dur>0：进度条 + 「剩余 Xs / 总Ys」+ hint「结束后设备自动复位阀门」+ 按钮禁用
  │    │      dur=0&pulse>0：kv「执行中：设备达到脉冲数后自动结束并复位阀门」+ 按钮恢复（新命令立即覆盖）
  │    │      dur=0&pulse=0：kv「无自动结束：持续执行，直至下发新命令（新命令立即覆盖）」+ 按钮恢复
  │    ├─ [完成] ok banner「✓ 任务完成 · 阀门已自动复位」3s 自动回落空闲（仅 dur>0 有确定完成时刻）
  │    └─ [忙] err banner「⏱ 设备忙…（实时任务被静默忽略，无 ACK）」2.6s（发送超时无 ACK 触发）
...
| 动作 | 命令 | 行为 |
|---|---|---|
| 立即执行 | 0x03（载荷 6B：阀+开关+时长2B+脉冲2B） | 校验 0–65535 → 发送 → ACK 状态机（见上树）。固件语义（control_center.c:262）：执行=保持命令状态至 时长到 或 脉冲打满，**结束时设备自动反向复位阀门且无任何上报**——App 倒计时为本地镜像；忙=仅定时任务执行中（无 ACK 静默忽略→超时横幅）；执行中新 0x03 合法（立即覆盖当前动作），故 dur=0 态表单/按钮保持可用
