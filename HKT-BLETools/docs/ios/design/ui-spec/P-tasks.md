...
body
  ...
  │    │      dur>0：进度条（=剩余量，自满格随倒计时递减，点下即可见；宽度逐秒变化带 0.25s 缓动过渡=原型 .progress i 的 transition:width .25s）+「剩余 Xs / 总Ys · 结束后设备自动复位阀门」；执行中按钮保持可用（新命令立即覆盖=提前改状态）
...
1. 入口：SVC100 详情页设备操作区「阀门任务」瓷砖（TSK 徽标，副文案「实时任务 · 定时任务表」；DC/UDS 不显示）。2026-09-14 用户裁决：入口属产品 UI——原型详情页已同步补齐（此前仅演示工具栏可达），iOS 已按本条实现。
