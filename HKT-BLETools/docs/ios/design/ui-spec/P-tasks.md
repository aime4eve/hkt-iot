...
body
  ...
  │    ├─ linkbtn「▸ 高级：定时回位与脉冲」（默认折叠）：展开=持续时间(s)+rtAdvDurHint / 脉冲数+rtAdvPulseHint
...
| 动作 | 命令 | 行为 |
|---|---|---|
| 阀行点按 开/关 | 0x03（载荷 6B：阀+开关+时长2B+脉冲2B；高级项默认 0/0=纯开关保持） | 校验 0–65535 → 发送 → ACK 状态机（见上树）。固件语义（control_center.c:262）：执行=保持命令状态至 时长到 或 脉冲打满，**结束时设备自动反向复位阀门且无任何上报**——App 倒计时为本地镜像；忙=仅定时任务执行中（无 ACK 静默忽略→超时横幅）；执行中新 0x03 合法（立即覆盖当前动作）→ 开关面板执行中按钮不锁定，可随时反向
