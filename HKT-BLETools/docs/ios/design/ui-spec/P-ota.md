...
### stage 2–7 进行中（用户 2026-09-10 裁决：标注目标设备 + 内容居中；2026-09-24 裁决：**升级过程不允许取消**，原「取消升级」贴底按钮与离开保护弹窗废弃）
- 整列 flex 纵向：**stageList 顶部** → 中段卡片+横幅（`margin:auto 0` 垂直居中）；无任何取消/离开入口
...
- 升级中页面无返回入口（navbar backText 空）、系统返回/侧滑返回一律拦截，直至成功（stage 8）/失败（stage 9）才恢复出口；引擎超时/断链等异常自行转入失败页
...
1. 演示引擎：Timer 380ms tick 复刻原型推进（无 Scene 复位/版本不符演示分支；失败分支经引擎超时触发，文案就绪；2026-09-24 起升级中无手动离开路径）。
