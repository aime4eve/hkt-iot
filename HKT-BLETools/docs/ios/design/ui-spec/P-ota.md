...
- 当前阶段卡：阶段文案（数据传输中/数据发送完成/等待设备重启…/等待设备重新广播…/正在重新连接设备…/正在读取新版本…）
  - stage≥4 追加 warn badge「尚未完成」；stage==2 页写入时追加「· 页写入中…」
  - 进度条（stage≥4 满，否则 pkt%）；kv 左「包 X / 1284」右「NN%」（stage≤3）
- stage≤3：warn banner「⚠︎ 升级期间请保持 App 前台、勿锁屏」
- 「取消升级」.btn danger → guard 对话框：⚠︎ 升级正在进行 / leaveB / 继续升级(secondary) / 仍然离开(danger→fail)
