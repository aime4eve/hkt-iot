...
9. SVC100 valve tasks (2026-09-06, verified from `SVC communicate.c`): realtime `0x03` payload = valve(0 both/1/2) + state(0/1) + duration(2 B BE, seconds, 0 = no auto-stop) + pulse(3 B BE); **busy (local schedule running) is silently ignored — no ACK**, so the app must treat a 2 s timeout as "rejected: busy". Timed task `0x04` payload = id(1-16) + valve(0-2) + state(0/1) + pulse(2 B BE) + start/end minute-of-day (2 B BE each) + repeat mask ≤ 0x7F, **bit0=Monday … bit6=Sunday** (`(wday?wday:7)-1`); any invalid field ⇒ silent return without ACK; valid ⇒ persisted to flash + ACK. Delete `0x05` payload = id 1-16, `0xFF` = delete all (force-stops a running task); ACK always. The schedule table has **no BLE read-back** — `0x3D` task sync exists only on the LoRaWAN platform path (`Device_ReportLocalTaskData` → uplink ring buffer), so the app must keep a local mirror and say so in the UI. BLE transparent-bridge module model/config (vendor-side, absent from MCU source); EPS100 build baseline (`#if 1` branch artifact); `ble_sleep_mode` retention across reboots (not observed persisted).
