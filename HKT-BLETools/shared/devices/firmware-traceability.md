...
11. UDS100 power `0xFE` / calibration `0xFD` / config `0x02` **[App mitigated 2026-09-20; firmware RX robustness OPEN — ledger FD-004]**: write frames sent while the 1 Hz query response is still being assembled (device single RX buffer, 50 ms idle framing, query handler blocks hundreds of ms on ranging/ADC) are silently dropped — no ACK, no effect; retries succeed. App now suspends polling for the whole calibration window and around power write attempts (probe-verified: 0xFD immediate ACK + "Calibration Done" text land first-try). Frame contract unchanged: `hkt 00 seq 0005 FE 00 00 00 0v crc` (value in last payload byte = firmware `data[10]`). ⚠️ Android's `streamDevice(0xFE,…)` payload is always `FFFFFFFF` (`data[10]=0xFF` matches neither branch) — Android never actually toggled UDS100 power; do not copy.
