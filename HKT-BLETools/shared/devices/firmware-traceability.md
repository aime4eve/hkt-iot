...
> **缺陷状态以 `HKT-Firmwares/docs/固件缺陷台账.md` 为唯一权威**（2026-09-20 起）。本节条目保留 iOS↔固件契约事实，不更新缺陷生命周期。
...
7. Bootloader ACK = `hkt cmd(1) count(2 BE) bootload(8)` (14 B); first request is (0x02, 0); a cmd-0xFF packet forces the flash flush (page = 2048 B, so a partial last page only flushes on 0xFF). **[Revised 2026-09-11, protocol audit]** The size-threshold completion path only ACKs (0x03,0) and does NOT jump — the jump to app happens exclusively on the host **finish frame `hkt 0001 03 crc bootload` (cmd=3, uart.c:410-422 AppProgramRun)**; omitting it leaves the device in the bootloader forever (update flag is only cleared by the new app after it boots). iOS engine sends finish on ACK(3,0).
10. BLE time sync `0x06` **[Revised 2026-09-11, protocol audit — reachability differs per family]**:
    - SVC100: guard `data[5]==4 && data[6]==6` at top level (communicate.c:1585) — reachable, answers ACK, offset = configured timezone (incl. half-hour codes) and re-arms the schedule clock. **The declared len field must be 4 (data length only, NOT cmd+data)**; `stamp==0` silently ignored. iOS: hkt frame + ACK wait.
    - UDS100 / DC200Family: the same guard is **dead code** (nested inside / shadowed by outer `data[5]==5||9` / `==5||4` gates — UDS-comm:1183 vs :1280, DC-comm:899 vs :987), so the hkt frame can never work there. The only working path is the ASCII command `syncDeviceTimestamp:<unix>`: no ACK, firmware adds a **fixed UTC+8** (UDS-comm:1093), and **DC's ASCII path additionally writes the wrong month (tm_mon+1, DC-comm:1014 — firmware defect, do not use until fixed)**. iOS: UDS sends the ASCII command (outcome "sent, no ack"), DC time sync is disabled.
    - App requirement unchanged: send phone Unix seconds; iOS must NOT pre-apply any timezone offset.
...
8b. ~~OPEN firmware defect~~ **RESOLVED — SVC100 realtime-task `0x03` pulse width (firmware team CONFIRMED 2026-09-09; fix verified in working tree same day)**: the `data[5]==7 && data[6]==3` branch now reads the pulse as 2 bytes (`real_time_task_t.pulse_count = srcHex[5] << 8 | srcHex[6]`) at BOTH sites — BLE entry `communicate.c:1660` and shared dispatch case `DATATYPE_SET_CLOCK_SCHEDULE` `communicate.c:556` — matching the len-7 gate (= cmd + 6-byte payload: valve/state/time(2B)/pulse(2B)). App contract unchanged: send exactly 6 payload bytes (fixture `TX-SVC-TASK-001`, byte-identical to Android V3.21). History: pre-fix firmware read 3 bytes, over-reading the CRC high byte (`(sent_pulse<<8)|crcHi`, e.g. send 100 → device got ~25 787). NOTE (2026-09-09): the firmware working tree path is now double-nested `HKT-Firmwares/in-house/HKT-Firmwares/in-house/...` — flagged to user. See item 9.
...
11. UDS100 power on/off `0xFE` **[OPEN firmware defect — shipped V2.10 lacks the OFF branch; HEAD source already fixed, needs a new build]** (verified live on device 2026-09-20, probe log): app frame = `hkt 00 seq 0005 FE 00 00 00 0v crc` (len 5 = cmd + 4-byte payload, value in the LAST payload byte ⇒ firmware reads `data[10]`) — byte-compatible with the Android frame length (`streamDevice` len = cmd+data = 5). Device under test runs SW `0x0A` = **V2.10** (read live via TLV `0x01` = `02.0A`, UDS100_AU915_V2.10_26.6.24). Verified behavior: power-ON `data[10]=0x01` → ACK frame (`hkt 00 seq FF`, 7 B) received, `power_on=1` persisted, next poll `0x8D`=1 → detail page switches ✓. Power-OFF `data[10]=0x00` → **silently ignored twice (no ACK), `power_on` stays 1** — V2.10 has no OFF branch. Firmware HEAD (`UDS communicate.c` ~:1219-1233) already contains both `0x01`/`0x00` branches + **unconditional `callback_BLEAck()`** — firmware team only needs to cut a new release. App behavior is correct as-is: honest "no ACK" banner after the 700 ms retry; contract unchanged. ⚠️ Android evidence: `streamDevice(0xFE, 0)` payload is ALWAYS `FFFFFFFF` (`String.format("%08X", 0xFF)`, dataLen 4) for BOTH its POWER_ON_START and POWER_OFF_START events ⇒ `data[10]=0xFF` matches neither branch even on HEAD — **per source + V2.10 behavior, Android never actually toggles UDS100 power** (it treats the unconditional ACK as success). Do NOT copy Android's payload. Note: one transient ON failure (no ACK, unreproduced) was observed once right after a fresh connect on an earlier app build; the succeeding test ACKed immediately.
