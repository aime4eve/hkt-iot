...
8b. **OPEN firmware defect — SVC100 realtime-task `0x03` pulse width (2026-09-07, awaiting firmware-team ruling; user decision: file as todo, App side unchanged)**: the `data[5]==7 && data[6]==3` branch (BLE entry `communicate.c:1649`, shared dispatch case `DATATYPE_SET_CLOCK_SCHEDULE` `communicate.c:556`) reads the pulse as 3 bytes (`srcHex[5]<<16|srcHex[6]<<8|srcHex[7]`) while the len-7 gate only carries a 6-byte payload, so `srcHex[7]` is the CRC high byte and the on-device pulse becomes `(sent_pulse<<8)|crcHi` (e.g. send 100 → device gets ~25 787). **Recommended fix (backward compatible): read 2 bytes (`pulse = srcHex[5]<<8|srcHex[6]`) in BOTH places, keep the len-7 gate** — Android V3.21 frames keep working unchanged. Do NOT widen the gate to 8: that would silently drop every shipped Android frame. Evidence: Android `Communicate.kt` cmd 0x03 block (len=7, `%02X%02X%04X%04X` = 6-byte payload); iOS fixture `TX-SVC-TASK-001`; byte-walk in the 2026-09-07 review. See item 9.
