/**
 * HKT GM-100 LoRaWAN Smart Gas Meter — payload decoder v1.1.0 (TTN)
 * Same core as gm-100_chirpstack_v1.1.0.js; decodeUplink(input{bytes,fPort}) entry
 * serves both the platform adapter and a legacy Decoder(bytes, port) alias below.
 *
...
/** Legacy TTN entry: Decoder(bytes, port) delegating to the same core. */
function Decoder(bytes, port) {
  var r = decodeUplink({ bytes: bytes, fPort: port });
  return r.errors && r.errors.length ? { data: r.data, errors: r.errors } : r.data;
}

if (typeof module !== "undefined" && module.exports) {
  module.exports = { decodeUplink: decodeUplink, Decoder: Decoder, gm100Decode: gm100Decode };
}
