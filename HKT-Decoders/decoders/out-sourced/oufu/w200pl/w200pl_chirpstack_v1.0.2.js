/**
 * HKT W200PL（LoRaWAN 通用版本）手表 上行解码器
 * 依据（正确性来源）：《W200PL-Lorawan-通信协议-通用版本-V2.1.docx》§3、§4
 *
 * 帧结构：[0xBD][MSGID][payload][checksum 1B]（与 B2315L V2.1 同构，无 DEVID；
 *   请求校准时间报文 ff00ff 除外）。校验和 = 0xFF - Σ(payload)，文档注明
 *   "通用版本不强求校验"且示例值与其算法不尽一致——宽松处理：仅消费不校验。
 *
 * 与 B2315L V2.1 的差异：
 *   - 新增 0xE9 设备状态（开机上报一笔；下行修改定位/健康上报频率后上报）：
 *     type u8 + len u16LE + locationFlag u8 + locationFreqMin u16LE +
 *     healthFlag u8 + healthFreqMin u16LE
 *   - 本文档无 0xA5 组号（那是 Lora 私有版 V2.3 的帧，私有版解码器见
 *     w200pl_private_v1.0.0.js，帧带 8 字节 DEVID）
 *   - 其余帧（02/21/B5/03/D6/C3/F9/BB/32TLV/C0/28）文档示例帧与
 *     B2315L V2.1 逐字节一致，直接复用同名解析逻辑
 *
 * 版本历史：
 *   v1.0.0  2026-09-17 基于 b2315l_chirpstack_v2.1.1.js 复制，新增 0xE9；
 *           保留其对 A2313 紧凑帧（无 BD 头）的自动兼容检测。
 *
 * ==== 以下为继承自 b2315l v2.1.1 的原始 changelog ====
 */
...
function decodeUplink(input) {
    ...
    while (offset < bytes.length) {
        ...
        switch (bytes[idPos]) {
            ...
            case 0xE9: consumed = decodeDeviceStatus(bytes, hOffset); break;
...
function decodeAlarm2(bytes, offset) {
    if (bytes.length - offset < frameNeed(6)) return -1;
    var alarmType = readU16LE(bytes, offset + 2);
    decoded.WarnType = alarmType.toString(16);
    if (alarmType === 5) {
        // 健康阈值报警：type(2) ts(4) contentLen(2) + n*3（类型1B+方向1B+值i16LE 2B）
        if (bytes.length - offset < frameNeed(11)) return -1;
        var ts5 = readU32LE(bytes, offset + 4);
        var len5 = readU16LE(bytes, offset + 8);
        if (len5 % 4 !== 0 || bytes.length - offset < frameNeed(11 + len5)) return -1;
        var items = [];
        for (var i = 0; i < len5; i += 4) {
            var p = offset + 10 + i;
            items.push({
        ...
        decoded.timestamp = dateFormat(ts5);
...
// W200PL LoRaWAN V2.1 §4.3.2: BD E9 typeU8 lenU16LE locationFlagU8 locFreqU16LE healthFlagU8 healthFreqU16LE CK
// 开机上报一笔；下行修改上报频率后上报一笔。len 恒为 6。
function decodeDeviceStatus(bytes, offset) {
    if (bytes.length - offset < frameNeed(12)) return -1;
    decoded.status_type = bytes[offset + 2];
    var len = readU16LE(bytes, offset + 3);
    if (len < 6 || bytes.length - offset < frameNeed(4 + len)) return -1;
    decoded.locationFreqModified = bytes[offset + 5] === 1;
    decoded.locationFreqMin = readU16LE(bytes, offset + 6);
    decoded.healthFreqModified = bytes[offset + 8] === 1;
    decoded.healthFreqMin = readU16LE(bytes, offset + 9);
    return 6 + len;
}
