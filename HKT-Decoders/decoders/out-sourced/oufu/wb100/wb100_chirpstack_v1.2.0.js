// ============================================================================
// HKT WB100 LoRaWAN wristband - ChirpStack uplink payload codec.
//
// ================================ CHANGELOG =================================
// 2026-09-09 v1.3（现场脚本基线，入库为 v1.0.0）
// 2026-09-18 v1.1.0（平台）- dateFormat 固定北京时间（UTC+8）。
// 2026-09-18 v1.2.0（平台）- 非法帧从 warning 改为 error 并终止解析：
//   未知 MessageId / 帧头截断 / 消息截断 / 0xC1 无 payload 定义 / 0xBA 非法
//   flag·type（黄金样例守卫 × 文档 Message ID 表裁定；黄金样例回归全绿）。
// ============================================================================
...
function decodeUplink(input) {
    ...
    while (offset < bytes.length) {
        ...
        if (!MSG_IDS[bytes[offset]]) {
            errors.push("offset " + offset + ": unknown MessageId 0x" + hex8(bytes[offset]) + ", stop parsing");
            return { data: {}, errors: errors };
        }
        if (bytes.length - offset < 2) {
            errors.push("truncated frame header at offset " + offset);
            return { data: {}, errors: errors };
        }
        ...
        switch (bytes[offset]) {
            ...
            case 0xBA: consumed = decodeTemperature(bytes, offset, errors); break;
            case 0x02: consumed = decodeAlarm(bytes, offset); break;
            case 0xD6: consumed = decodeLbe(bytes, offset); break;
            case 0xC1:
                errors.push("offset " + offset + ": 0xC1 has no payload definition in V1.3");
                return { data: {}, errors: errors };
            default:
                errors.push("offset " + offset + ": unknown MessageId 0x" + hex8(bytes[offset]));
                return { data: {}, errors: errors };
        }
        if (consumed < 0) {
            if (!errors.length) {
                errors.push("truncated MessageId 0x" + hex8(bytes[offset]) + " frame at offset " + offset);
            }
            return { data: {}, errors: errors };
        }
        offset += consumed;
...
function decodeTemperature(bytes, offset, errors) {
    if (bytes.length - offset < 2) return -1;
    var flag = bytes[offset + 1];
    if (flag !== 0x00 && flag !== 0x01) {
        errors.push("temperature flag 0x" + hex8(flag) + " not defined in V1.3");
        return -1;
    }
    var pos = offset + 2;
    ...
    if (type !== 0x01 && type !== 0x02) {
        errors.push("temperature type 0x" + hex8(type) + " not defined in V1.3");
        return -1;
    }
...
function dateFormat(timestamp) {
    // 协议时间戳按北京时间（UTC+8）呈现。固定偏移 + getUTC*，
    // 不依赖解码器宿主时区（v1.0.0 现场脚本在 UTC 宿主上偏 8 小时，v1.1.0 修复；
    // 与 b2315l v1.0.3 同款修复）。
