...
// 2026-09-22 v1.0.4 - Accept the BD-framed 0xBB version frame WITHOUT the
//   trailing checksum byte. A production band on EU868 answered the version
//   query with "BD BB 10 ...36" (19 bytes, version string "C2310AL.868AO.06" -
//   same model family as the manual's own 4.3.1 example "C2310L.470.AA.01"):
//   the length byte says 16, exactly 16 ASCII bytes follow, and the frame
//   simply ends - no CK. v1.0.3 required the CK slot (guard 4+len) and
//   rejected the frame as truncated with no output.
//  16. [fix] decodeFirmwareVersion: when the checksum slot is missing or holds
//      the next frame's 0xBD header, consume 3+len instead. Documented frames
//      (with CK, e.g. the manual example ending 1A) and compact frames decode
//      unchanged.
//  17. [note] Other MessageIds from this firmware line may share the no-CK
//      trait; not extended speculatively - add tolerance per frame type when
//      real captures arrive.
// ============================================================================
...
// 4.3.1 BD BB lenU8 ascii[len] CK (4 + len bytes). Production firmware also
// sends this frame without the checksum byte (v1.0.4, real capture
// "C2310AL.868AO.06"): treat it as checksum-less when only 3+len bytes remain
// or the checksum slot holds the next frame's 0xBD header.
function decodeFirmwareVersion(bytes, offset) {
    if (bytes.length - offset < frameNeed(3)) return -1;
    var len = bytes[offset + 2];
    var noChecksum = !compactFrame &&
        (bytes.length - offset < 4 + len || bytes[offset + 3 + len] === 0xBD);
    if (noChecksum && bytes.length - offset < 3 + len) return -1;
    var version = "";
    for (var i = 0; i < len; i++) version += String.fromCharCode(bytes[offset + 3 + i]);
    decoded.Version_len = len;
    decoded.Version = version;
    return noChecksum ? 3 + len : 4 + len;
}
...
function dateFormat(timestamp) {
    ...
    // 不依赖解码器宿主时区（v1.0.2 在 UTC 宿主上偏 8 小时，由服务器回归暴露，v1.0.3 修复）。
