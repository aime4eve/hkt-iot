/**
 * Payload Decoder — 牛羊定位器（ChirpStack v4）
 *
 * Copyright 2023 HKT SmartHard
 *
 * @product HKT-CT02（与 GAT-100 同一固件协议）
 *
 ...
 *
 * ── 修改日志 ─────────────────────────────────────────────
 * 2022    v1.0.0 初版（旧脚本，缺陷：0x15 步数按 1 字节解析、负经纬度符号失效、
 *         while(dataLen--) 截断死循环、仅覆盖 7 种 TLV 且未知类型静默错位）。
 * 2026-09-18 v1.1.0 按同源固件 LoRaWAN_Animals_Track 全量重写（黄金样例 × 固件逐字节互证），
 *         字段契约与 gat-100 chirpstack v1.1.0 一致：
