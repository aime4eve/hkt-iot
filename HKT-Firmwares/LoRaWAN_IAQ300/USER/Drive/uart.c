...
void UART1_IRQHandler(void)
{
    ...
    if ((ENABLE == UARTx_IER_RXBF_IE_Getable(UART1)) && (SET == UARTx_ISR_RXBF_Chk(UART1))) {
        u8 DataReceived = UARTx_RXBUF_Read(UART1); // 读取出来接收到的数据,接收中断标志仅可通过读取RXBUF寄存器清除
    }

    // 发送中断处理
    if ((ENABLE == UARTx_IER_TXSE_IE_Getable(UART1)) && (SET == UARTx_ISR_TXSE_Chk(UART1))) {
        UARTx_ISR_TXSE_Clr(UART1); // 清除发送中断标志
    }
}
...
void UART2_IRQHandler(void)
{
    ...
    if ((ENABLE == UARTx_IER_RXBF_IE_Getable(UART2)) && (SET == UARTx_ISR_RXBF_Chk(UART2))) {
        ...
        if (uartPmsa.recvLen < uart_recv_max_len) {
            uartPmsa.recvData[uartPmsa.recvLen++] = DataReceived;
            uartPmsa.recvTimeout = 100 / SYSTEM_TIMER_BASE; // 100ms超时
        }
    }

    // 发送中断处理
    if ((ENABLE == UARTx_IER_TXSE_IE_Getable(UART2)) && (SET == UARTx_ISR_TXSE_Chk(UART2))) {
        if (uartPmsa.txIndex < uartPmsa.sendLen) {
            UARTx_TXBUF_Write(UART0, uartPmsa.sendData[uartPmsa.txIndex]); // 发送一个数据
            uartPmsa.txIndex++;
        }
...
void UART3_IRQHandler(void)
{
    ...
    if ((ENABLE == UARTx_IER_RXBF_IE_Getable(UART3)) && (SET == UARTx_ISR_RXBF_Chk(UART3))) {
        u8 DataReceived = UARTx_RXBUF_Read(UART3); // 读取出来接收到的数据,接收中断标志仅可通过读取RXBUF寄存器清除
				if (uartCM.recvLen < uart_recv_max_len) {
            uartCM.recvData[uartCM.recvLen++] = DataReceived;
            uartCM.recvTimeout = 100 / SYSTEM_TIMER_BASE; // 100ms超时
        }
    }
...
void UART5_IRQHandler(void)
{
    ...
    if ((ENABLE == UARTx_IER_RXBF_IE_Getable(UART5)) && (SET == UARTx_ISR_RXBF_Chk(UART5))) {
        ...
        if (uartHCHO.recvLen < uart_recv_max_len) {
            uartHCHO.recvData[uartHCHO.recvLen++] = DataReceived;
            uartHCHO.recvTimeout = 100 / SYSTEM_TIMER_BASE; // 100ms超时
        }
    }

    // 发送中断处理
    if ((ENABLE == UARTx_IER_TXSE_IE_Getable(UART5)) && (SET == UARTx_ISR_TXSE_Chk(UART5))) {
        if (uartHCHO.txIndex < uartHCHO.sendLen) {
            UARTx_TXBUF_Write(UART5, uartHCHO.sendData[uartHCHO.txIndex]); // 发送一个数据
            uartHCHO.txIndex++;
        }
...
void Uartx_Init(UART_Type *UARTx, u32 BaudRate)
{
    ...
    case 3: // 进行UART3的初始化配置
        /*UART3 IO 配置*/
        AltFunIO(GPIOB, GPIO_Pin_14, 0); // PC10 UART3 RX
        AltFunIO(GPIOB, GPIO_Pin_15, 0); // PC11 UART3 TX
...
void LPUART0_IRQHandler(void)
{
    ...
    if ((ENABLE == LPUARTx_IER_TXSE_IE_Getable(LPUART0)) && (SET == LPUARTx_ISR_TXSE_Chk(LPUART0))) {
        ...
        if (uartLoRa.txIndex < uartLoRa.sendLen) {
            LPUARTx_TXBUF_Write(LPUART0, uartLoRa.sendData[txIndex]); // 发送一个数据
            uartLoRa.txIndex++;
        }
        // 清除发送中断标志
...
{
    static unsigned char s[1000]; // This needs to be large enough to store the string TODO Change magic number
    unsigned char *p;
    va_list args;
    int str_len;
    ...
    /* 互斥操作 */
    tx_mutex_get(&mutex_info, TX_WAIT_FOREVER);
    INFO_SendData(p, str_len);
    tx_mutex_put(&mutex_info);
}
