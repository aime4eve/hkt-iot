...
AT_Rsp_Typedef AT_Exec_SendCMD(const char *cmd, u16 timeout, const char *keyword)
{
    ...
    if (strstr((const char *)keyword, "+RTC INFO:") != NULL) {
        value += 6;
    } else if (strstr((const char *)keyword, "+LINK INFO:") != NULL) {
        value += 6;
    }
...
bool AT_LoRaWAN_syncTime(void)
{
    ...
    while (retry < 3) {
        if (AT_OK == AT_Exec_SendCMD(valid, 10000, "+RTC INFO:")) {
            DEBUG_TRACE(LOG_TAG, "%s", strstr((const char *)uartLoRa.recvData, "+RTC INFO:"));
            /* 获取有效数据 */
            memset(&splitBuf, 0, sizeof(splitBuf));
            char *validStr = strstr((const char *)uartLoRa.recvData, "+RTC INFO:");
            int validNum = 0, i = 0;
            cmd_split(validStr, " ", splitBuf, &validNum);
...
/**
 * @brief  设置RTC时间，并设置时区为东8区
 * @param
 * @retval 指令执行状态 FALSE 执行失败，TRUE 执行成功
 */
bool AT_LoRaWAN_setRTC(void)
{
    u16 retry = 0;
    char valid[] = "AT+RTC=25,03,01,00,00,00,+8";
    while (retry < 3) {
        if (AT_OK == AT_Exec_SendCMD(valid, 2500, "OK")) {
            return TRUE;
        }
