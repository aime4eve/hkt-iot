...
void OPM_PortCfg(void)
{
    ...
    /** 初始化 IO **/
    OutputIO(OPM_RST_PORT, OPM_RST_PIN, OUT_PUSHPULL);
    OutputIO(OPM_CSB_PORT, OPM_CSB_PIN, OUT_PUSHPULL);
    OutputIO(OPM_SDA_PORT, OPM_SDA_PIN, OUT_PUSHPULL);
    OutputIO(OPM_SCL_PORT, OPM_SCL_PIN, OUT_PUSHPULL);
    OutputIO(OPM_DC_PORT, OPM_DC_PIN, OUT_PUSHPULL);
    InputtIO(OPM_BUSY_PORT, OPM_BUSY_PIN, IN_NORMAL);
    EPD_PWR_CTRL_L;
}
...
void thread_display(ULONG thread_input)
{
    ...
    while (1) {
        ...
        tx_thread_sleep(3000); // 刷新完后最少等待3秒再进行刷新
        status = tx_event_flags_get(&event_group, /* 事件标志控制块 */
                                    BIT_DISPLAY, /* 等待标志 */
                                    TX_OR_CLEAR, /* 等待任意bit满足即可 */
                                    &actual_events, /* 获取实际值 */
