...
#define error(s, f, ...) INFO("\n%s %s: " s "\n", "LIS3DH", f, ##__VA_ARGS__)
#define error_dev(s, f, d, ...) INFO("\n%s %s: addr %02x - " s "\n", "LIS3DH", f, d->addr, ##__VA_ARGS__)
#else
#define error(s, f, ...)
...
lis3dh_sensor_t *lis3dh_init_sensor(uint8_t addr, uint8_t cs)
{
    lis3dh_sensor_t *dev;
                       ...
                       lis3dh_int_signal_t signal, bool value)
{
    ...
    // change the register
    switch (type) {
    case lis3dh_int_data_ready:
        ctrl3.IT_DRDY1 = value;
        break;

    case lis3dh_int_fifo_watermark:
        ctrl3.I1_WTM1 = value;
        break;

    case lis3dh_int_fifo_overrun:
        ctrl3.I1_OVERRUN = value;
        break;

    case lis3dh_int_event1:
        if (signal == lis3dh_int1_signal)
            ctrl3.I1_AOI1 = value;
        else
            ctrl6.I2_AOI1 = value;
        break;

    case lis3dh_int_event2:
        if (signal == lis3dh_int1_signal)
            ctrl3.I1_AOI2 = value;
        else
            ctrl6.I2_AOI2 = value;
    ...
    default:
        dev->error_code = LIS3DH_WRONG_INT_TYPE;
                                 ...
                                 lis3dh_int_event_gen_t gen)
{
    ...
    case lis3dh_wake_up:
        ...
        intx_cfg.SIXD = 0;
        break;
    case lis3dh_free_fall:
        intx_cfg.AOI = 1;
        intx_cfg.SIXD = 0;
        break;

    case lis3dh_4d_movement:
        d4d_int = true;
    case lis3dh_6d_movement:
        intx_cfg.AOI = 0;
        intx_cfg.SIXD = 1;
        break;

    case lis3dh_4d_position:
        d4d_int = true;
    case lis3dh_6d_position:
        intx_cfg.AOI = 1;
        intx_cfg.SIXD = 1;
        break;
    }
                    ...
                    uint16_t *adc1, uint16_t *adc2, uint16_t *adc3)
{
    ...
    if (adc1)
        *adc1 = lsb_msb_to_type(int16_t, data, 0) >> (ctrl1.LPen ? 8 : 6);
    if (adc2)
        *adc2 = lsb_msb_to_type(int16_t, data, 2) >> (ctrl1.LPen ? 8 : 6);
...
static bool lis3dh_spi_read(lis3dh_sensor_t *dev, uint8_t reg, uint8_t *data, uint16_t len)
{
    ...
    // Spi2WriteReadNByte(mosi, 1, miso, len + 1);
    // shift data one by left, first byte received while sending register address is invalid
    //    for (int i = 0; i < len; i++)
    //        data[i] = miso[i + 1];
    Spi2WriteReadNByte(mosi, 1, miso, len);
    for (int i = 0; i < len; i++)
        data[i] = miso[i];

#ifdef LIS3DH_DEBUG_LEVEL_2
    INFO("LIS3DH %s: read the following bytes from reg %02x: ", __FUNCTION__, reg);
    for (int i = 0; i < len; i++)
        INFO("%02x ", data[i]);
    INFO("\n");
#endif
...
#ifdef LIS3DH_DEBUG_LEVEL_2
    INFO("LIS3DH %s: Write the following bytes to reg %02x: ", __FUNCTION__, reg);
    for (int i = 1; i < len + 1; i++)
        INFO("%02x ", mosi[i]);
    INFO("\n");
#endif
    Spi2WriteReadNByte(mosi, len + 1, NULL, 0);
    return true;
}
...
static bool lis3dh_i2c_read(lis3dh_sensor_t *dev, uint8_t reg, uint8_t *data, uint16_t len)
{
    ...
    int result = i2c_slave_read(dev->addr, reg, data, len);
...
#ifdef LIS3DH_DEBUG_LEVEL_2
    INFO("LIS3DH %s: Read following bytes: ", __FUNCTION__);
    INFO("%02x: ", reg & 0x7f);
    for (int i = 0; i < len; i++)
        INFO("%02x ", data[i]);
    INFO("\n");
...
static bool lis3dh_i2c_write(lis3dh_sensor_t *dev, uint8_t reg, uint8_t *data, uint16_t len)
{
    ...
    int result = i2c_slave_write(dev->addr, reg, data, len);
...
#ifdef LIS3DH_DEBUG_LEVEL_2
    INFO("LIS3DH %s: Wrote the following bytes: ", __FUNCTION__);
    INFO("%02x: ", reg & 0x7f);
    for (int i = 0; i < len; i++)
        INFO("%02x ", data[i]);
    INFO("\n");
