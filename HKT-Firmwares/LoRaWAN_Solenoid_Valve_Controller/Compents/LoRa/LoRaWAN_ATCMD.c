...
bool AT_LoRaWAN_getStatus(void)
{
    u16 retry = 0;
    while (retry < 3) {
        if (AT_OK == AT_Exec_SendCMD("at+join", 1500, "+join,ok")) {
            DEBUG_TRACE(LOG_TAG, "%s", strstr((const char *)uartLoRa.recvData, "+join,ok"));
            return TRUE;
        }
        retry++;
        LoRa_DelayMs(1000);
