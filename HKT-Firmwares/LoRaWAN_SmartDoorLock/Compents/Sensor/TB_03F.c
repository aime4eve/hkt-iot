...
static AT_Rsp_Typedef AT_Exec_SendCMD(const char *cmd, u16 timeout, const char *keyword)
{
    ...
    Ble_SendCMD(cmd);
    Ble_SendCMD("\r\n");
    char *value = (char *)uartBle.recvData;
...
bool AT_Ble_getVer(void)
{
    ...
    while (retry < 3)
    {
        ...
        {
            DEBUG_TRACE(LOG_TAG, "%s", uartBle.recvData);
            return true;
        }
        retry++;
        tx_thread_sleep(1000);
...
bool AT_Ble_getMac(void)
{
    ...
    while (retry < 3)
    {
        ...
        {
            DEBUG_TRACE(LOG_TAG, "%s", strstr((const char *)uartBle.recvData, "+BLEMAC"));

            /* 获取有效数据 */
            memset(&splitBuf, 0, sizeof(splitBuf));
            char *validStr = strstr((const char *)uartBle.recvData, "+BLEMAC");
            int validNum = 0, i = 0;
...
{
    u16 retry = 0;
    char value[] = "AT+BLENAME=HKT_SDL_0392";
    sprintf(value, "AT+BLENAME=HKT_SDL_%02X%02X", BLEMAC[4], BLEMAC[5]);
    while (retry < 3)
    {
        if (AT_OK == AT_Exec_SendCMD(value, 1500, "OK"))
        {
...
bool AT_Ble_getAdvIntv(void)
{
    ...
    while (retry < 3)
    {
        if (AT_OK == AT_Exec_SendCMD("AT+BLEADVINTV?", 1500, "OK"))
        {
            DEBUG_TRACE(LOG_TAG, "%s", uartBle.recvData);
            return true;
        }
 ...
 * @retval 指令执行状态 false 执行失败，true 执行成功
 */
bool AT_Ble_getAdvData(void)
{
    u16 retry = 0;
    while (retry < 3)
    {
        if (AT_OK == AT_Exec_SendCMD("AT+BLEADVDATA?", 1500, "OK"))
        {
            DEBUG_TRACE(LOG_TAG, "%s", uartBle.recvData);
            return true;
        }
 ...
 * @retval 指令执行状态 false 执行失败，true 执行成功
 */
bool AT_Ble_setAdvData(u16 mtu)
{
    u16 retry = 0;
    char value[] = "AT+BLEADVDATA=200";
    sprintf(value, "AT+BLEADVDATA=%d", mtu);
    while (retry < 3)
    {
        if (AT_OK == AT_Exec_SendCMD(value, 1500, "OK"))
        {
...
bool AT_Ble_getBleState(void)
{
    ...
    while (retry < 3)
    {
        ...
        {
            DEBUG_TRACE(LOG_TAG, "%s", uartBle.recvData);
            char *state = strstr("BLESTATE:", (char *)uartBle.recvData) + 9;
            if (state[0] == 0x30)
            {
...
/**
 * @brief  蓝牙模组运行状态变化
 * @param  mode 0：进入浅睡眠并且下次电不自动进入浅睡眠状态
                1：进入浅睡眠并且下次电自动进入浅睡眠状态
                2：进入深度睡眠模式
 * @retval
 */
