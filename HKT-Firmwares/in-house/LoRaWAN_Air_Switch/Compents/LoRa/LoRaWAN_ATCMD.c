...
bool AT_LoRaWAN_getDEVEUI(void)
{
    ...
    while (retry < 3) {
        if (AT_OK == AT_Exec_SendCMD("AT+DEVEUI?", 1500, "+DEVEUI:")) // 设置为OTAA激活，开启热启动
        {
            ...
            DEBUG_TRACE(LOG_TAG, "DevEUI: %02x %02x %02x %02x %02x %02x %02x %02x", LoRaWAN.DevEUI[0], LoRaWAN.DevEUI[1], LoRaWAN.DevEUI[2],
...
bool AT_LoRaWAN_setDEVEUI(void)
{
    ...
    sprintf(DevEUI_Str, "AT+DEVEUI= %02x %02x %02x %02x %02x %02x %02x %02x, D3 91 01 02 20 10 28 16, 1", LoRaWAN.DevEUI[0], LoRaWAN.DevEUI[1],
            LoRaWAN.DevEUI[2], LoRaWAN.DevEUI[3], LoRaWAN.DevEUI[4], LoRaWAN.DevEUI[5], LoRaWAN.DevEUI[6], LoRaWAN.DevEUI[7]);
    while (retry < 3) {
...
bool AT_LoRaWAN_setAPPEUI(void)
{
    ...
    sprintf(AppEUI_Str, "AT+APPEUI= %02x %02x %02x %02x %02x %02x %02x %02x", LoRaWAN.AppEUI[0], LoRaWAN.AppEUI[1], LoRaWAN.AppEUI[2],
...
bool AT_LoRaWAN_getAPPEUI(void)
{
    ...
    while (retry < 3) {
        if (AT_OK == AT_Exec_SendCMD("AT+APPEUI?", 1500, "OK")) {
            ...
            DEBUG_TRACE(LOG_TAG, "APPEUI: %02x %02x %02x %02x %02x %02x %02x %02x", LoRaWAN.AppEUI[0], LoRaWAN.AppEUI[1], LoRaWAN.AppEUI[2],
                        LoRaWAN.AppEUI[3], LoRaWAN.AppEUI[4], LoRaWAN.AppEUI[5], LoRaWAN.AppEUI[6], LoRaWAN.AppEUI[7]);
            return TRUE;
        }
        retry++;
...
#if 1
    /* 设置为密钥可读模式 */
    sprintf(AppKEY_Str, "AT+APPKEY= %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x,0", LoRaWAN.AppKEY[0],
            LoRaWAN.AppKEY[1], LoRaWAN.AppKEY[2], LoRaWAN.AppKEY[3], LoRaWAN.AppKEY[4], LoRaWAN.AppKEY[5], LoRaWAN.AppKEY[6], LoRaWAN.AppKEY[7],
            LoRaWAN.AppKEY[8], LoRaWAN.AppKEY[9], LoRaWAN.AppKEY[10], LoRaWAN.AppKEY[11], LoRaWAN.AppKEY[12], LoRaWAN.AppKEY[13], LoRaWAN.AppKEY[14],
            LoRaWAN.AppKEY[15]);
#else
    /* 设置为密钥不可读模式 */
    sprintf(AppKEY_Str, "AT+APPKEY= %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x,1", LoRaWAN.AppKEY[0],
            LoRaWAN.AppKEY[1], LoRaWAN.AppKEY[2], LoRaWAN.AppKEY[3], LoRaWAN.AppKEY[4], LoRaWAN.AppKEY[5], LoRaWAN.AppKEY[6], LoRaWAN.AppKEY[7],
            LoRaWAN.AppKEY[8], LoRaWAN.AppKEY[9], LoRaWAN.AppKEY[10], LoRaWAN.AppKEY[11], LoRaWAN.AppKEY[12], LoRaWAN.AppKEY[13], LoRaWAN.AppKEY[14],
            LoRaWAN.AppKEY[15]);
#endif
...
bool AT_LoRaWAN_setDEVADDR(void)
{
    ...
    sprintf(DevADDR, "AT+DEVADDR= %02x %02x %02x %02x", LoRaWAN.DevADDR[0], LoRaWAN.DevADDR[1], LoRaWAN.DevADDR[2], LoRaWAN.DevADDR[3]);
    while (retry < 3) {
...
bool AT_LoRaWAN_setAPPSKEY(void)
{
    ...
    sprintf(AppSKEY_Str, "AT+APPSKEY= %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x", LoRaWAN.AppSKEY[0],
...
bool AT_LoRaWAN_setNwkSKEY(void)
{
    ...
    sprintf(AppSKEY_Str, "AT+NWKSKEY= %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x", LoRaWAN.NwkSKEY[0],
...
bool AT_LoRaWAN_setJOIN_Close(int isABP)
{
    ...
    if (AT_OK == AT_Exec_SendCMD(queryStr, 1500, targetMode) &&
        strstr((const char *)uartLoRa.recvData, "otaa,0")) {
...
#if FUNC_LoRaWAN_US915_0
		sprintf(valid, "at+ch=us915,1,0,3");
#else
    sprintf(valid, "at+ch=us915,2,0,3");
#endif
#elif FUNC_LoRaWAN_AU915
#if FUNC_LoRaWAN_AU915_0
		sprintf(valid, "at+ch=au915,1,0,3");
#else
...
bool AT_LoRaWAN_setAPPEUI(void)
{
    ...
    char key_s[20] = {0};
    sprintf(key_s, "%02x%02x%02x%02x%02x%02x%02x%02x",
            LoRaWAN.AppEUI[0], LoRaWAN.AppEUI[1], LoRaWAN.AppEUI[2], LoRaWAN.AppEUI[3],
            LoRaWAN.AppEUI[4], LoRaWAN.AppEUI[5], LoRaWAN.AppEUI[6], LoRaWAN.AppEUI[7]);
...
bool AT_LoRaWAN_setAPPKEY(void)
{
    ...
    char key_s[50] = {0};
    sprintf(key_s, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
            LoRaWAN.AppKEY[0], LoRaWAN.AppKEY[1], LoRaWAN.AppKEY[2], LoRaWAN.AppKEY[3],
            LoRaWAN.AppKEY[4], LoRaWAN.AppKEY[5], LoRaWAN.AppKEY[6], LoRaWAN.AppKEY[7],
            LoRaWAN.AppKEY[8], LoRaWAN.AppKEY[9], LoRaWAN.AppKEY[10], LoRaWAN.AppKEY[11],
            LoRaWAN.AppKEY[12], LoRaWAN.AppKEY[13], LoRaWAN.AppKEY[14], LoRaWAN.AppKEY[15]);
...
bool AT_LoRaWAN_getDEVADDR(void)
{
    ...
    while (retry < 3) {
        if (AT_OK == AT_Exec_SendCMD("at+devaddr=?", 1500, "\r\n")) {
            DEBUG_TRACE(LOG_TAG, "%s", (const char *)uartLoRa.recvData);
            /* 转换字符串为hex */
            cmd_get_hexstr2hex(LoRaWAN.DevADDR, 4, uartLoRa.recvData + 15, 8);
...
bool AT_LoRaWAN_setAPPSKEY(void)
{
    ...
    if (AT_OK == AT_Exec_SendCMD("at+appskey=?www.lanvee.com", 1500, key_s)) {
...
bool AT_LoRaWAN_setNwkSKEY(void)
{
    ...
    if (AT_OK == AT_Exec_SendCMD("at+nwkskey=?www.lanvee.com", 1500, key_s)) {
 ...
 * @retval 指令执行状态 FALSE 执行失败，TRUE 执行成功
 */
bool AT_LoRaWAN_getTranscnf(void)
{
    u16 retry = 0;
    while (retry < 3) {
        if (AT_OK == AT_Exec_SendCMD("at+transcnf=?", 1500, "\r\n")) {
            DEBUG_TRACE(LOG_TAG, "%s", (const char *)uartLoRa.recvData);
            return TRUE;
        }
        retry++;
...
/**
 * @brief 设置/查询终端 LoRaWAN 上行透传消息类型,复位生效
 * @param on（需要应答），off（不需要应答）
 * @retval 指令执行状态 FALSE 执行失败，TRUE 执行成功
 */
bool AT_LoRaWAN_setTranscnf(uint8_t is_cnf)
{
    u16 retry = 0;

    char str[10] = {0};
    char cmd[30] = {0};
    snprintf(str, sizeof(str), "%s", is_cnf ? "on" : "off");
    snprintf(cmd, sizeof(cmd), "at+transcnf=%s", is_cnf ? "on" : "off");

    // 先查询当前设置
    if (AT_OK == AT_Exec_SendCMD("at+transcnf=?", 1500, "\r\n")) {
        if (strstr((const char *)uartLoRa.recvData, str)) {
            DEBUG_TRACE(LOG_TAG, "Current JoinAdr is already OFF, no need to configure");
            return FALSE;
        }
    }
...
bool AT_LoRaWAN_setSleepoff(void)
{
    u16 retry = 0;

    // 先查询当前设置
    if (AT_OK == AT_Exec_SendCMD("at+sleep=?", 1500, "off")) {
        if (strstr((const char *)uartLoRa.recvData, "off")) {
            DEBUG_TRACE(LOG_TAG, "Current JoinAdr is already OFF, no need to configure");
            return FALSE;
        }
    ...
    while (retry < 3) {
        if (AT_OK == AT_Exec_SendCMD("at+sleep=off", 1500, "ok")) {
            DEBUG_TRACE(LOG_TAG, "%s", (const char *)uartLoRa.recvData);
            return TRUE;
        }
        retry++;
...
bool AT_LoRaWAN_setSleepLight(void)
{
    u16 retry = 0;

    // 先查询当前设置
    if (AT_OK == AT_Exec_SendCMD("at+sleep=?", 1500, "light,0,")) {
        if (strstr((const char *)uartLoRa.recvData, "off")) {
            DEBUG_TRACE(LOG_TAG, "Current JoinAdr is already OFF, no need to configure");
            return FALSE;
        }
    ...
    while (retry < 3) {
        if (AT_OK == AT_Exec_SendCMD("at+sleep=light,0,level,low", 1500, "ok")) {
            DEBUG_TRACE(LOG_TAG, "%s", (const char *)uartLoRa.recvData);
            return TRUE;
        }
        retry++;
...
// 上送消息类型：cnf（需要应答），uncnf（不需要应答）
bool AT_LoRaWAN_macTxHex(const char *cnf, uint8_t fport, const char *data)
{
    char cmd_buf[256] = {0};
    snprintf(cmd_buf, sizeof(cmd_buf), "at+mactxhex=%s,%d,%s", cnf, fport, data);

    // 先查询当前设置
    if (AT_OK == AT_Exec_SendCMD(cmd_buf, 10000, "ok")) {
        return TRUE;
    }
...
bool AT_LoRaWAN_setRxFmt(uint8_t is_raw)
{
    u16 retry = 0;

    char str[10] = {0};
    char cmd[30] = {0};
    snprintf(str, sizeof(str), "%s", is_raw ? "raw" : "hex");
    snprintf(cmd, sizeof(cmd), "at+rxfmt=%s", is_raw ? "raw" : "hex");

    // 先查询当前设置
    if (AT_OK == AT_Exec_SendCMD("at+rxfmt=?", 1500, "\r\n")) {
        if (strstr((const char *)uartLoRa.recvData, str)) {
            DEBUG_TRACE(LOG_TAG, "Current Rx Fmt is already %s, no need to configure",str);
            return FALSE;
        }
    }
