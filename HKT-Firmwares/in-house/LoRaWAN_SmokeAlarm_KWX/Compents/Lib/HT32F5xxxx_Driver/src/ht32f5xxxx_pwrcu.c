...
void PWRCU_DeepSleep2Ex(PWRCU_SLEEP_ENTRY_Enum SleepEntry)
{
 ...
 if (HT_EXTI->WAKUPFLG)
 {
   HT_EXTI->WAKUPCR &= ~(0xFFFF);//WAKEUP复位
   HT_EXTI->WAKUPFLG = 0xFFFF;//唤醒标志位标志为唤醒
   HT_RTC->CR &= (~1UL);//RTC控制归寄存器全为0
   HT_RTC->CMP = 1;//RTCCNT寄存器复位到0，重新开始计数
   HT_RTC->IWEN = (1 << 9);//比较匹配唤醒使能
   HT_RTC->CR = (1 << 4) | (1 << 2) | (1 << 0);//RTCOUT 输出保持在有效电平,
   __SEV();
   __WFE();
   __WFE();
   HT_RTC->CR &= (~1UL);//RTC控制归寄存器全为0
   rw((u32)&HT_RTC->SR);//读取RTC状态寄存器

   HT_RTC->CMP = uBackUp[0];//将备份数据恢复到RTC
