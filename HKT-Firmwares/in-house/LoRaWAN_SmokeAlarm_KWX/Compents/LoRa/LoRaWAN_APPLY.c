...
void rstLoRaWANModule(void)
{
    ...
    LoRa_DelayMs(5);
    GPIO_WriteOutBits(LoRaWAN_NRST_PORT, LoRaWAN_NRST_PIN, SET);
    LoRa_DelayMs(200);
    LoRaWAN.joinState = 0;
}
...
{
    if (GPIO_ReadOutBit(LoRaWAN_MODE_PORT, LoRaWAN_MODE_PIN) != RESET)
        return SET;
    else
        return RESET;
}
...
{
    GPIO_WriteOutBits(LoRaWAN_MODE_PORT, LoRaWAN_MODE_PIN, (FlagStatus)status);
    LoRa_DelayMs(5);
}
...
{
    if (GPIO_ReadOutBit(LoRaWAN_WAKE_PORT, LoRaWAN_WAKE_PIN) != RESET)
        return SET;
    else
        return RESET;
}
...
{
    GPIO_WriteOutBits(LoRaWAN_WAKE_PORT, LoRaWAN_WAKE_PIN, (FlagStatus)status);
    LoRa_DelayMs(5);
}
...
{
    GPIO_WriteOutBits(LoRaWAN_NRST_PORT, LoRaWAN_NRST_PIN, (FlagStatus)status);
    LoRa_DelayMs(5);
}
...
{
    if (GPIO_ReadInBit(LoRaWAN_BUSY_PORT, LoRaWAN_BUSY_PIN) != RESET)
        return SET;
    else
        return RESET;
}
...
{
    if (GPIO_ReadInBit(LoRaWAN_STAT_PORT, LoRaWAN_STAT_PIN) != RESET)
        return SET;
    else
        return RESET;
}
...
{
    uint32_t Device_Serial0 = HT_FLASH->MDID;
    uint32_t Device_Serial1 = HT_FLASH->DID;
    uint32_t Device_Serial2 = HT_FLASH->CID[0];
...
void LoRaWAN_Init(void)
{
    ...
    AT_LoRaWAN_setFREQBW(1, 2, 916600000, 200); // 上下同频
    AT_LoRaWAN_setRX2(2, 916600000);
    // AT_LoRaWAN_setCHMASK(0x00FF);
    AT_LoRaWAN_setADR(0);
    AT_LoRaWAN_setDataRate(3);
#elif FUNC_LoRaWAN_AS923_2 
    AT_LoRaWAN_setBAND(94);
    // AT_LoRaWAN_saveEEPROM(); // 保存配置参数
    // LoRa_DelayMs(20);
    // rstLoRaWANModule();
    // AT_LoRaWAN_setFREQBW(1, 8, 921400000, 200); // 上下同频
    // AT_LoRaWAN_setRX2(2, 921400000);
...
#elif FUNC_LoRaWAN_AS923_3
    snprintf(sfre, sizeof(sfre), "AS923_2");
#else
...
void LoRaWAN_JoinInit(void)
{
    ...
    case LoRaWAN_JOINEVENT_INIT:
        ...
        setLoRaWANStatus(LoRaWAN_WAKE_STATUS); // 唤醒模块
        setLoRaWANMode(LoRaWAN_CMD_MODE);      // 配置进入命令模式
                                               // #if FUNC_LoRaWAN_AS923_3
                                               //         AT_LoRaWAN_setFREQBW(1, 2, 916600000, 200); // 上下同频
                                               //         AT_LoRaWAN_setRX2(2, 916600000);
                                               //         AT_LoRaWAN_setCHMASK(0x0300);
                                               //         AT_LoRaWAN_saveEEPROM(); // 保存配置参数
                                               // #endif
        // AT_LoRaWAN_setJOIN();
        rstLoRaWANModule(); // 复位模组
    ...
    case LoRaWAN_JOINEVENT_JOINED:
        ...
        //         setLoRaWANStatus(LoRaWAN_WAKE_STATUS);      // 唤醒模块
        //         setLoRaWANMode(LoRaWAN_CMD_MODE);           // 配置进入命令模式
        //         AT_LoRaWAN_setFREQBW(1, 8, 915400000, 200); // 上下同频
        //         AT_LoRaWAN_setCHMASK(0xFF00);
        //         AT_LoRaWAN_saveEEPROM(); // 保存配置参数
        //         AT_LoRaWAN_getFREQ();
        //         setLoRaWANMode(LoRaWAN_PASSTHROUGH_MODE); // 配置进入透传模式
