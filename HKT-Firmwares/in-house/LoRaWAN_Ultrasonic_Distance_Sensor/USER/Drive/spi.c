...
void Spi2Init(void)
{
    ...
    GPIO_InitStruct.Pin = LL_GPIO_PIN_12; // NSS
...
void Spi2WriteReadNByte(uint8_t *wdata, u16 wlength, uint8_t *rdata, u16 rlength)
{
    ...
    LL_GPIO_ResetOutputPin(GPIOB, LL_GPIO_PIN_12); // 选中
    ...
    if (wlength) {
        for (i = 0; i < wlength; i++) {
            ...
            LL_SPI_TransmitData8(SPI2, wdata[i]);
        ...
        if (!Txtimeout) {
            INFO("\n Write Tx Timeout SPI2->SR %04x %s\n", SPI2->SR, __func__);
        }
        if (!Bsytimeout) {
            INFO("\n Write Bsy Timeout SPI2->SR %04x %s\n", SPI2->SR, __func__);
        }
        if (!Rxtimeout) {
            INFO("\n Write Recv Timeout SPI2->SR %04x %s\n", SPI2->SR, __func__);
        }
    }
    ...
    if (rlength) {
        for (i = 0; i < rlength; i++) {
            ...
            rdata[i] = LL_SPI_ReceiveData8(SPI2);
        }
        if (!Txtimeout) {
            INFO("\n Write Tx Timeout SPI2->SR %04x %s\n", SPI2->SR, __func__);
        }
