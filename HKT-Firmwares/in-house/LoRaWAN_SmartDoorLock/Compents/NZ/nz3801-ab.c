 ...
 * @param
 */
void nz3801SetTimer(u32 fc) // domod TPrescalEven bit=0
{
    u32 prescale = 0;
    u32 t;
 ...
 * @param
 */
u8 nz3801SoftPwrDown(bool bEnable)
{
    if (bEnable)
    {
 ...
 * @param
 */
u8 nz3801ActivateField(bool activateField)
{
    u8 reg_val;
 ...
 */
u8 nz3801Transceive(eCmd command,
 ...
 * @param
 */
u8 nz3801IBLOCK(const u8 *inf, u16 infLength, u8 *response, u16 *responseLength)
{
    u8 rec[256];
...
#define SendRB(type)                \
    if (++timer >= 3 + Rerr)        \
        return ERR_TIMEOUT;         \
    RB[0] = 0xa2 | type | BlockNum; \
    r = nz3801Transceive(TA_RSBLOCK, RB, 1, 0, rec, &rlen, 0);
