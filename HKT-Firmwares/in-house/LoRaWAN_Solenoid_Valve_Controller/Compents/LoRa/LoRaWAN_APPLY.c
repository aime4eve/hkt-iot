...
void LoRaWAN_JoinInit(void)
{
    ...
    case LoRaWAN_JOINEVENT_REGISTER:
        if (getLoRaWANBusyIOLevel() && getLoRaWANStatIOLevel()) {
            ...
            if (factoryMode) {
                DEBUG_TRACE(ACK_TAG, "factoryMode SVC Controller Step:LoRa Register Success"); // 设备入网成功
            }
        } else {
            waitRegisterTick++;
...
void getJoinedStatus(void)
{
    ...
    int count;
    char *response = (char *)uartLoRa.recvData;
    if (strstr(response, "+join,ok") != NULL) {
        cmd_split(response + strlen("+join,ok") + 1, ",", result, &count);
        if (count >= 3) {
            // 将16进制字符串转换为有符号16进制整数
            rssi = atoi(result[0]);
            snr = atoi(result[1]);
