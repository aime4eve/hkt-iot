...
{
    u8 timeout = 200;
    if (device_t.sleepDelay < 1)
        device_t.sleepDelay = 1;
    I2Cx_BUF_Write(I2Cx, x_byte); // 写发送缓冲寄存器
    do {
        if (I2Cx_ISR_TXIF_Chk(I2Cx))
            break;
        TicksDelayUs(1);
    } while (timeout--);
...
void I2C_Init(I2C_Type *I2Cx)
{
    ...
    if (I2Cx == I2C0) {
        ...
        AltFunIO(GPIOA, GPIO_Pin_14, ALTFUN_OPENDRAIN_PULLUP); // PA14;//SCL
        AltFunIO(GPIOA, GPIO_Pin_15, ALTFUN_OPENDRAIN_PULLUP); // PA15;//SDA
    }
