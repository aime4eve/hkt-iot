...
void FUNC_KEY_SINGLE_CLICK_Handler(void)
{
    ...
    // sensorConvertTimestamp = Timestamp + 5;
    ...
    if (device_t.sensor_t.air_level >= AIR_LEVEL_MILD && (device_t.beepEnable || device_t.ledShowMode)) {
...
void FUNC_KEY_LONG_RRESS_START_Handler(void)
{
    ...
    if (!poweroff_status) {
        ...
        device_t.pirEvent = 0;
        uartLoRa.sendDelay = 0;
        device_t.sleepDelay = 0;
    } else if (poweroff_status == 2) {
        poweroff_status = 0;
        NVIC_SystemReset();
    }
}
