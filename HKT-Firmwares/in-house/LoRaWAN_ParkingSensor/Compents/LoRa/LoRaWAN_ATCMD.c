...
AT_Rsp_Typedef AT_Exec_SendCMD(const char *cmd, u16 timeout, const char *keyword)
{
    ...
    // {
    //     /* 状态错误 */
    //     DEBUG_TRACE(ERROR_TAG, "LoRaWAN Busy IO Level ERROE!");
    //     return AT_ERROR;
    // }
...
bool AT_LoRaWAN_getStatus(void)
{
    u16 retry = 0;
    while (retry < 3) {
        if (AT_OK == AT_Exec_SendCMD("AT+STATUS?", 1500, "+STATUS:")) {
            DEBUG_TRACE(LOG_TAG, "%s", strstr((const char *)uartLoRa.recvData, "+STATUS:"));
            return TRUE;
        }
        retry++;
        LoRa_DelayMs(1000);
...
{
    u16 retry = 0;
    char valid[] = "AT+FREQ=2,4,868900000,869100000,864100000,864300000";
    while (retry < 3) {
        if (AT_OK == AT_Exec_SendCMD(valid, 1500, "OK")) {
            return TRUE;
        }
