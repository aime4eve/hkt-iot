...
void SystemClock_Config(void)
{
   ...
   while (LL_RCC_LSE_IsReady() != 1) {
    ...
    // SCB->CPACR |= (3 << 20) | (3 << 22); // 启用 FPU 单精度浮点运算
}
...
// 设置一个延迟并检查是否已经过去
bool check_delay_expired(uint32_t startTime, uint32_t delayMs)
{
    uint32_t currentTime = get_syspant_ms();
    uint32_t elapsedTime = (currentTime >= startTime) ? (currentTime - startTime) : (UINT32_MAX - startTime + currentTime + 1);
...
void Systick_1ms_Handler(void)
{
    ...
    if (SysTicker >= SEC_DELAY) {
        ...
        Timestamp++;
        SystemTimer();
        if (!uartLoRa.sendDelay && !device_t.park_delay) {
            if (device_t.sleep_delay && !factoryMode)
