...
void ADC_BatterySglConvert(void)
{
    ...
    u16 batteryVoltage = 3000 * VREFINT_CAL / convertedValue;
    device_t.sensor_t.vol = batteryVoltage;

    if (batteryVoltage - 2500 > 0 && batteryVoltage < 3100) {
        batteryLevel = (batteryVoltage - 2600) * 2 / 10;
    ...
    DEBUG_TRACE(LOG_TAG, "Get Battery %dmV, Level %d\n", batteryVoltage, batteryLevel);
}
...
void ADC_BatterySglConvert(void)
{
    ...
    do {
        ...
        if (timeout) {
            timeout--;
            if (!timeout) {
                DEBUG_TRACE(WARN_TAG, "LL_ADC_IsCalibrationOnGoing Timeout");
                break;
            }
        }
    } while (LL_ADC_IsCalibrationOnGoing(ADC1));
    ...
    do {
        ...
        if (timeout) {
            timeout--;
            if (!timeout) {
                DEBUG_TRACE(WARN_TAG, "convertedValue Timeout");
                break;
            }
        }
