...
void EPD_ShowUpdate()
{
    ...
    case 0:                                                               // 温湿度 空气质量等级 CO2
...
#if EPD_SW_CO_HCHO_SHOW
        Paint_DrawBitMap_Paste(Image_HCHO_1, 20, 60, 40, 18, flip_color);
        /*告警图标显示*/
        if (device_t.sensor_t.hcho_ppm >= 80 && device_t.sensor_t.hcho_ppm < 200) {
        ...
        EPD_Custom_Number24_Show_HCHO(120, 90, device_t.sensor_t.hcho_ppm);
        Paint_DrawBitMap_Paste(Image_MG_M3_1, 210, 100, 40, 12, flip_color);

        Paint_DrawLine(co2_x_start, co2_y_start, co2_x_start, co2_y_end, foreground_color, DOT_PIXEL_1X1, LINE_STYLE_SOLID); // 垂直分割线
        Paint_DrawLine(co2_x_end, co2_y_start, co2_x_end, co2_y_end, foreground_color, DOT_PIXEL_1X1, LINE_STYLE_SOLID);     // 垂直分割线
        Paint_DrawLine(co2_x_start, co2_y_start, co2_x_end, co2_y_start, foreground_color, DOT_PIXEL_1X1, LINE_STYLE_SOLID); // 水平分割线

        EPD_Custom_Number12_Show_HCHO(co2_x_start - (11 * 5 - 5), co2_y_end, 200);
        EPD_Custom_Number12_Show_HCHO(co2_x_start - (11 * 5 - 5), co2_y_start - 16, 0);

        for (x = 0; x < 36; x++) {
            if (device_t.hcho_history_buf[x] < 0) {
                co2_high = 1;
            } else if (device_t.hcho_history_buf[x] < 200) {
                co2_high = (device_t.hcho_history_buf[x]) * 0.6;
            } else // MAX
            {
                co2_high = 120;
            }
    ...
    case 1:                                                               // 温湿度 空气质量等级 CO2  TVOC 光照 大气压
        ...
        EPD_Custom_Number24_Show_HCHO(120, 80, device_t.sensor_t.hcho_ppm);
        Paint_DrawBitMap_Paste(Image_MG_M3_1, 210, 90, 40, 12, flip_color);
        ...
        for (x = 0; x < 36; x++) {
            if (device_t.hcho_history_buf[x] < 0) {
                ...
                co2_high = (device_t.hcho_history_buf[x]) * 2 / 3 * 0.6;
            } else // MAX
            {
                co2_high = 80;
            }
...
#if EPD_SW_CO_HCHO_SHOW
        EPD_Custom_Number24_Show_Other(315, 127, device_t.sensor_t.co2);
        Paint_DrawBitMap_Paste(Image_CO2_1, 292, 107, 32, 10, flip_color); // 甲醛
        Paint_DrawBitMap_Paste(Image_PPM_1, 357, 150, 32, 10, flip_color); // 图标
#else
...
void EPD_Custom_Number24_Show_HCHO(u16 x, u16 y, u16 data)
{
    ...
    index = hcho_ppm_l / 100;
    Paint_DrawBitMap_Paste(number24_index[index].data_buf, x_start, y_start, number24_index[index].width, number24_index[index].height, flip_color);
    x_start += number24_index[index].width;

    index = hcho_ppm_l / 10 % 10;
    Paint_DrawBitMap_Paste(number24_index[index].data_buf, x_start, y_start, number24_index[index].width, number24_index[index].height, flip_color);
    x_start += number24_index[index].width;
...
void EPD_Custom_Number12_Show_HCHO(u16 x, u16 y, u16 data)
{
    u16 x_start = x, y_start = y;
    ...
    Paint_DrawNum(x_start, y_start, index, &Font_12_18, foreground_color, background_color);
    x_start += 12;

    x_start += DOT_PIXEL_2X2;
    Paint_DrawPoint(x_start, y_start + 18 - 3, foreground_color, DOT_PIXEL_2X2, DOT_STYLE_DFT);
    x_start += DOT_PIXEL_2X2;
...
void EPD_Custom_Number12_Show_HCHO(u16 x, u16 y, u16 data)
{
    ...
    x_start += DOT_PIXEL_1X1;
    Paint_DrawPoint(x_start, y_start + 18 - 3, foreground_color, DOT_PIXEL_1X1, DOT_STYLE_DFT);
    // x_start += DOT_PIXEL_1X1;
...
void thread_display(ULONG thread_input)
{
    ...
    while (1) {
        ...
        tx_thread_sleep(1500);                        // 刷新完后最少等待3秒再进行刷新
        status = tx_event_flags_get(&event_group,     /* 事件标志控制块 */
                                    BIT_DISPLAY,      /* 等待标志 */
                                    TX_OR_CLEAR,      /* 等待任意bit满足即可 */
                                    &actual_events,   /* 获取实际值 */
