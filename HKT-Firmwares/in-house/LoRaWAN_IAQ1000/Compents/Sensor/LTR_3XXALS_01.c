...
u8 ltr3xx_serial_number(u8 *serial_number)
{
    ...
    // error = i2c_hard_write(LTR3XX_I2C_DEV_ID, &buffer[0], offset);
    // if (error)
    // {
    //     return error;
    // }
    // sensirion_i2c_hal_sleep_usec(10000);
    // error = i2c_hard_read(LTR3XX_I2C_DEV_ID, &buffer[0], 1);
    // if (error)
    // {
    //     return error;
    // }
    error = i2c_hard_write_read(LTR3XX_I2C_DEV_ID, LTR3XX_PART_ID, &buffer[0], 1);
    if (error)
    {
        return error;
    }
...
u8 ltr3xx_get_data_ready_flag(u8 *status)
{
    ...
    error = i2c_hard_write_read(LTR3XX_I2C_DEV_ID, LTR3XX_ALS_STATUS, &buffer[0], 1);
    if (error)
    {
        return error;
    }
...
u8 ltr3xx_get_measurement_rate(u8 *measurement)
{
    ...
    // sensirion_i2c_hal_sleep_usec(10000);
    // error = i2c_hard_read(LTR3XX_I2C_DEV_ID, &buffer[0], 1);
    error = i2c_hard_write_read(LTR3XX_I2C_DEV_ID, LTR303_MEAS_RATE, &buffer[0], 1);
    if (error)
    {
        return error;
    }
...
u8 ltr3xx_get_data(u8 *data, u8 reg)
{
    ...
    // sensirion_i2c_hal_sleep_usec(10000);
    // error = i2c_hard_read(LTR3XX_I2C_DEV_ID, &buffer[0], 1);
    error = i2c_hard_write_read(LTR3XX_I2C_DEV_ID, reg, &buffer[0], 1);
    if (error)
    {
        return error;
    }
...
void readLightIntensity(void)
{
    ...
    //    }
    // tx_thread_sleep(100);
    u8 ret = ltr3xx_get_data_ready_flag(&status);
    if (ret)
    {
    ...
    if ((status & 0x80) == 0) //已获得新数据且数据有效
    {
        ...
        if (device_t.sensor_t.light_data < 50)
        {
        ...
        else if (device_t.sensor_t.light_data < 1000)
        {
