 ...
 * @retval 指令执行状态 FALSE 执行失败，TRUE 执行成功
 */
bool AT_LoRaWAN_setMode(int miniRF)
{
    u16 retry = 0;
    char valid[20] = {0};
    sprintf(valid, "AT+WAP=%d", miniRF);
    while (retry < 3) {
        if (AT_OK == AT_Exec_SendCMD(valid, 2500, "OK")) {
            return TRUE;
        }
 ...
 * @retval 指令执行状态 FALSE 执行失败，TRUE 执行成功
 */
bool AT_MINIRF_setRadio(int freq, int power, int sf)
{
    u16 retry = 0;
    char valid[20] = {0};
    sprintf(valid, "AT+RADIO=%d,1,%d,%d", freq, power, sf);
    while (retry < 3) {
        if (AT_OK == AT_Exec_SendCMD(valid, 2500, "OK")) {
            return TRUE;
        }
 ...
 * @retval 指令执行状态 FALSE 执行失败，TRUE 执行成功
 */
bool AT_MINIRF_getRadio(void)
{
    u16 retry = 0;
    while (retry < 3) {
        if (AT_OK == AT_Exec_SendCMD("AT+RADIO?", 1500, "OK")) {
            DEBUG_TRACE(LOG_TAG, "%s", strstr((const char *)uartLoRa.recvData, "+RADIO:"));
            return TRUE;
        }
        retry++;
        LoRa_DelayMs(1000);
