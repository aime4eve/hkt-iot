...
void LoRaWAN_JoinInit(void)
{
    ...
    case LoRaWAN_JOINEVENT_REGISTER:
        if (getLoRaWANBusyIOLevel() && getLoRaWANStatIOLevel())
        {
            ...
            {
                DEBUG_TRACE(ACK_TAG, "factoryMode Cattle Sheep Track Step:LoRa Register Success"); // 设备入网成功
            }
            else
            {
                DEBUG_TRACE(LOG_TAG, "Cattle Sheep Track LoRa Register Success"); // 设备入网成功
            }
        }
        else
        {
            ...
            if (waitRegisterTick >= 240) // 120秒
            {
                ...
                if (factoryMode)      // 生产测试模式，直接退出
                {
                    DEBUG_TRACE(ACK_TAG, "factoryMode Cattle Sheep Track Step:LoRa Register Fail");
                    LoRa_DelayMs(500);
                    DEBUG_TRACE(ACK_TAG, "factoryMode Cattle Sheep Track Step:Exit");
    ...
    case LoRaWAN_JOINEVENT_JOINED:
        ...
        // mDelay(1000);
        // ledGreenBlinkTime = 2 * SEC_DELAY;
        device_t.syncDeviceState = 1;
        if (factoryMode)
        {
            factoryMode = 0;
            LoRa_DelayMs(1000);
            DEBUG_TRACE(ACK_TAG, "factoryMode Cattle Sheep Track Step:Finish"); // 测试完成
            LoRa_DelayMs(500);
        }
        BEEP_RUN(500, 30);
        LoRa_DelayMs(500);
