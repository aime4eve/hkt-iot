...
void LoRaWAN_JoinInit(void)
{
    ...
    case LoRaWAN_JOINEVENT_REGISTER:
        if (getLoRaWANBusyIOLevel() && getLoRaWANStatIOLevel()) {
            ...
            if (factoryMode) {
                LoRa_DelayMs(500);
                DEBUG_TRACE(ACK_TAG, "factoryMode People Counter Sensor Step:LoRa Register Sucess"); // 设备入网成功
            } else {
                DEBUG_TRACE(LOG_TAG, " People Counter Sensor LoRa Register Sucess"); // 设备入网成功
            }
        } else {
            waitRegisterTick++;
            ...
            if (waitRegisterTick >= 240) // 120秒
            {
                ...
                {
                    LoRa_DelayMs(500);
                    DEBUG_TRACE(ACK_TAG, "factoryMode People Counter Sensor Step:LoRa Register Fail");
                    LoRa_DelayMs(500);
                    DEBUG_TRACE(ACK_TAG, "factoryMode People Counter Sensor Step:Exit");
    ...
    case LoRaWAN_JOINEVENT_JOINED:
        if (factoryMode) {
            factoryMode = 0;
            LoRa_DelayMs(1000);
            DEBUG_TRACE(ACK_TAG, "factoryMode People Counter Sensor Step:Finish"); // 测试完成
            LoRa_DelayMs(500);
        }
        LoRaWAN.joinState = 1;
        jion_fail_cnt = 0;
