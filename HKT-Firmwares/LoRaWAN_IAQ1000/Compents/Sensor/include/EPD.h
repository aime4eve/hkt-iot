...
#define EPD_RST_H GPIO_SetBits(EPD_RST_PORT, EPD_RST_PIN)
#define EPD_RST_L GPIO_ResetBits(EPD_RST_PORT, EPD_RST_PIN)

#define EPD_CSB_PORT GPIOE
// #define EPD_CSB_PIN GPIO_Pin_7
#define EPD_CSB_PIN GPIO_Pin_8
#define EPD_CSB_H GPIO_SetBits(EPD_CSB_PORT, EPD_CSB_PIN)
#define EPD_CSB_L GPIO_ResetBits(EPD_CSB_PORT, EPD_CSB_PIN)

#define EPD_SDA_PORT GPIOE
// #define EPD_SDA_PIN GPIO_Pin_6
#define EPD_SDA_PIN GPIO_Pin_5
#define EPD_SDA_H GPIO_SetBits(EPD_SDA_PORT, EPD_SDA_PIN)
#define EPD_SDA_L GPIO_ResetBits(EPD_SDA_PORT, EPD_SDA_PIN)
#define READ_EPD_SDA_STATUS GPIO_ReadInputDataBit(EPD_SDA_PORT, EPD_SDA_PIN)
#define EPD_SDA_OUT OutputIO(EPD_SDA_PORT, EPD_SDA_PIN, OUT_PUSHPULL); // gpio 配置为输出
#define EPD_SDA_IN InputtIO(EPD_SDA_PORT, EPD_SDA_PIN, IN_NORMAL);     // gpio 配置为输入

#define EPD_SCL_PORT GPIOE
// #define EPD_SCL_PIN GPIO_Pin_5
#define EPD_SCL_PIN GPIO_Pin_7
#define EPD_SCL_H GPIO_SetBits(EPD_SCL_PORT, EPD_SCL_PIN)
#define EPD_SCL_L GPIO_ResetBits(EPD_SCL_PORT, EPD_SCL_PIN)

#define EPD_DC_PORT GPIOE
#define EPD_DC_PIN GPIO_Pin_4
#define EPD_DC_H GPIO_SetBits(EPD_DC_PORT, EPD_DC_PIN)
#define EPD_DC_L GPIO_ResetBits(EPD_DC_PORT, EPD_DC_PIN)

#define EPD_BUSY_PORT GPIOE
#define EPD_BUSY_PIN GPIO_Pin_3
#define READ_EPD_BUSY_STATUS GPIO_ReadInputDataBit(EPD_BUSY_PORT, EPD_BUSY_PIN)

#define DELAY_TIME 8 // 图片显示完停留时间(单位:秒) 主频升高8倍，时间是1/8。再加上补偿送数据减少的时间。

// 测试图
#define PIC_WHITE 255                  // 全白
#define PIC_BLACK 252                  // 全黑
#define PIC_A			1
#define PIC_B   	    2
#define PIC_HLINE		3
#define PIC_VLINE	    4
